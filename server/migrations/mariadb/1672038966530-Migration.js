...
const { dbname } = (await queryRunner.query("SELECT DATABASE() AS `dbname`"))[0];
        ...
        await queryRunner.query(`CREATE TABLE \`MeetingRespondent\` (\`RespondentID\` int NOT NULL AUTO_INCREMENT, \`MeetingID\` int NOT NULL, \`UserID\` int NULL, \`GuestName\` varchar(255) NULL, \`GuestEmail\` varchar(255) NULL, \`Availabilities\` text NOT NULL, INDEX \`IDX_7683aab7b4f83cd884c396b9ed\` (\`UserID\`), UNIQUE INDEX \`IDX_84550e253a24bf39b6d37b7839\` (\`MeetingID\`, \`UserID\`), PRIMARY KEY (\`RespondentID\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`MicrosoftCalendarCreatedEvent\` ADD CONSTRAINT \`FK_4af9eb16ff51bbcf86bac0c6c5c\` FOREIGN KEY (\`RespondentID\`) REFERENCES \`MeetingRespondent\`(\`RespondentID\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`GoogleCalendarCreatedEvent\` ADD CONSTRAINT \`FK_066ecb0b9e5f8491fc1d28d5fe9\` FOREIGN KEY (\`RespondentID\`) REFERENCES \`MeetingRespondent\`(\`RespondentID\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
