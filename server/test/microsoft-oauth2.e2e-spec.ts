  ...
  return mockAgent
    ...
    .reply(({ headers, path }) => {
      ...
      const params = decodeQueryParams(
        path.slice(eventsDeltaApiPath.length + 1),
      );
      return cb({ params, headers: headers as Record<string, string> });
    });
}

const authzEndpointPrefix =
...
const responseScopes =
  'openid profile email https://graph.microsoft.com/Calendars.ReadWrite';
const nonce = 'abcdef';
...
describe('OAuth2Controller (e2e) (Microsoft)', () => {
  ...
  it('/api/signup-with-microsoft (POST)', async () => {
    ({ token: token1 } = await signupNewUserWithMicrosoft(
  ...
  it('/api/signup-with-microsoft (POST) (account already exists)', async () => {
    ...
    expect(redirect2).toStrictEqual(
      'http://cabbagemeet.internal/error?e=E_OAUTH2_ACCOUNT_ALREADY_LINKED&provider=MICROSOFT',
    );
  });
  ...
  it('/api/signup-with-microsoft (POST) (not all scopes granted)', async () => {
    ...
    expect(redirect2).toStrictEqual(
      'http://cabbagemeet.internal/error?e=E_OAUTH2_NOT_ALL_SCOPES_GRANTED&provider=MICROSOFT',
    );
  });
  ...
  it('/api/login-with-microsoft (POST) (user exists, not linked yet, refresh token is present)', async () => {
    ...
    expect(
      redirect2.startsWith(
        'http://cabbagemeet.internal/confirm-link-microsoft-account?',
      ),
    ).toBe(true);
    const redirect2Params = decodeQueryParams(
      redirect2.slice(
        'http://cabbagemeet.internal/confirm-link-microsoft-account?'.length,
      ),
    );
    ...
    const { body: user } = await POST(
      '/api/confirm-link-microsoft-account',
      app,
      token,
    )
      .send({
  ...
  it('/api/me/microsoft-calendar-events (GET)', async () => {
    ...
    await GET(
      `/api/me/microsoft-calendar-events?meetingID=${meetingID}`,
      app,
      token,
    )
      .expect(HttpStatus.OK)
  ...
  it('/api/me/microsoft-calendar-events (GET) (refresh token)', async () => {
    const { sub, name, email, token } = await signupNewUserWithMicrosoft(
      {},
      app,
    );
    const { meetingID } = await createMeeting(
        ...
        value: [
          {
            ...
            start: {
              dateTime:
                schedule.startDateTime.slice(
                  0,
                  schedule.startDateTime.length - 1,
                ) + '.000000',
              timeZone: 'UTC',
            },
            end: {
              dateTime:
                schedule.endDateTime.slice(0, schedule.endDateTime.length - 1) +
                '.000000',
              timeZone: 'UTC',
            },
          },
