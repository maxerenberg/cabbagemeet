...
class Client {
  meeting: ServerMeeting | null = null;
  ...
  submitAvailabilities({dateTimes, userID = undefined, guestName = undefined}: SubmitAvailabilitiesArgs): Promise<SubmitAvailabilitiesResponse> {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        assert(this.meeting !== null);
        if (true) {
